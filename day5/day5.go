...
func processSeedsAndMapsP2(seedData []int, maps map[DataSetKey][]IdMap) uint64 {
	lowestLocation := uint64(2147483647)
	...
	for _, seedDataElement := range seedData {
		if !tikTok {
			seedBase = uint64(seedDataElement)
			tikTok = true
			continue
		} else {
			seedSpread = uint64(seedDataElement)
			tikTok = false
		}

		fmt.Printf("base %v spread %v\n", seedBase, seedSpread)
		...
		for seed := seedBase; seed < seedBase+seedSpread; seed++ {
			...
			for _, key := range allDataSetKeys {
				...
				// is id within any of this data set's maps
				for _, idMap := range maps[key] {
					if uint64(idMap.source) <= currentId && uint64(idMap.source)+uint64(idMap.spread) >= currentId {
						foundIdMap = idMap
						found = true
						// fmt.Printf("\t\tcurrentId %v\n", currentId)
						// fmt.Printf("\t\tfound %+v\n", foundIdMap)
				...
				if found {
					offset := currentId - uint64(foundIdMap.source)
					destination := uint64(foundIdMap.destination) + offset
					currentId = destination
					// fmt.Printf("\t\tresultId %v\n", destination)
				}
...
func parseInputP2(scanner *bufio.Scanner) ([]int, map[DataSetKey][]IdMap) {
	dataMap := make(map[DataSetKey][]IdMap)
	seeds := make([]int, 0)
